...
/** Minimal structural placeholder for KiCanvas's Project. The only uses of
 *  this field in this file are optional-chained reads of
 *  `root_schematic_page?.document`, so this shape is sufficient. The adapter
 *  does not populate `project`. */
type Project = { root_schematic_page?: { document?: unknown } };
import {
    At,
    Effects,
    Paper,
...
export class KicadSch {
    ...
    update_hierarchical_data(path?: string) {
        ...
        for (const s of this.symbols.values()) {
            ...
            const unit = instance_data.unit ?? s.unit;
        ...
        for (const s of this.sheets) {
            ...
            if (instance_data.page !== undefined) s.page = instance_data.page;
            if (instance_data.path !== undefined) s.path = instance_data.path;

            if (!s.instances.size) {
                const inst = new SchematicSheetInstance();
                if (instance_data.page !== undefined) inst.page = instance_data.page;
                if (instance_data.path !== undefined) inst.path = instance_data.path;
...
export class GraphicItem implements HasStrokeParams {
    parent?: LibSymbol | SchematicSymbol | undefined;
    private = false;
    stroke?: Stroke;
    fill?: Fill;
    uuid?: string;
